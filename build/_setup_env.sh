...
if [ -z "${KERNEL_DIR}" ]; then
    ...
    build_config_path=$(realpath ${BUILD_CONFIG})
...
export OUT_DIR=$(readlink -m ${COMMON_OUT_DIR}/$(basename ${KERNEL_DIR}))
...
if [ "${HERMETIC_TOOLCHAIN:-0}" -eq 1 ]; then
  ...
  # (e.g. debug info)
  export KCPPFLAGS="-ffile-prefix-map=${KERNEL_DIR}/= -ffile-prefix-map=${BASE_DIR}/="

  # set the common sysroot
  sysroot_flags+="--sysroot=${BASE_DIR}/build/build-tools/sysroot "
  ...
  ldflags+="-Wl,-rpath,${BASE_DIR}/prebuilts/kernel-build-tools/linux-x86/lib64 "
