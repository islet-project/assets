...
function create_modules_staging() {
  ...
  if [ -n "${modules_list_file}" ]; then
    ...
    if [[ -f "${BASE_DIR}/${modules_list_file}" ]]; then
      modules_list_file="${BASE_DIR}/${modules_list_file}"
    elif [[ "${modules_list_file}" != /* ]]; then
      echo "modules list must be an absolute path or relative to ${BASE_DIR}: ${modules_list_file}"
      exit 1
  ...
  if [ -n "${modules_blocklist_file}" ]; then
    ...
    if [[ -f "${BASE_DIR}/${modules_blocklist_file}" ]]; then
      modules_blocklist_file="${BASE_DIR}/${modules_blocklist_file}"
    elif [[ "${modules_blocklist_file}" != /* ]]; then
      echo "modules blocklist must be an absolute path or relative to ${BASE_DIR}: ${modules_blocklist_file}"
      exit 1
...
function build_vendor_dlkm() {
  ...
  local vendor_dlkm_modules_BASE_DIR=$(echo ${VENDOR_DLKM_STAGING_DIR}/lib/modules/*)
  local vendor_dlkm_modules_load=${vendor_dlkm_modules_BASE_DIR}/modules.load
  ...
  if [ -z "${VENDOR_DLKM_PROPS}" ]; then
    ...
    if [[ -f "${BASE_DIR}/${vendor_dlkm_props_file}" ]]; then
      vendor_dlkm_props_file="${BASE_DIR}/${vendor_dlkm_props_file}"
    elif [[ "${vendor_dlkm_props_file}" != /* ]]; then
      echo "VENDOR_DLKM_PROPS must be an absolute path or relative to ${BASE_DIR}: ${vendor_dlkm_props_file}"
      exit 1
...
if [ -n "${KMI_SYMBOL_LIST}" ]; then
  ${BASE_DIR}/build/copy_symbols.sh "$ABI_SL" "$KERNEL_DIR" \
...
if [[ -z "${SKIP_EXT_MODULES}" ]] && [[ -n "${EXT_MODULES_MAKEFILE}" ]]; then
  ...
  make -f "${EXT_MODULES_MAKEFILE}" KERNEL_SRC=${BASE_DIR}/${KERNEL_DIR} \
...
if [[ -z "${SKIP_EXT_MODULES}" ]] && [[ -n "${EXT_MODULES}" ]]; then
  ...
  for EXT_MOD in ${EXT_MODULES}; do
    ...
    EXT_MOD_REL=$(rel_path ${BASE_DIR}/${EXT_MOD} ${KERNEL_DIR})
    # The output directory must exist before we invoke make. Otherwise, the
    # build system behaves horribly wrong.
    mkdir -p ${OUT_DIR}/${EXT_MOD_REL}
    set -x
    make -C ${EXT_MOD} M=${EXT_MOD_REL} KERNEL_SRC=${BASE_DIR}/${KERNEL_DIR}  \
...
if [ -n "${MODULES}" ]; then
  ...
  if [ "${BUILD_INITRAMFS}" = "1" ]; then
    ...
    MODULES_BASE_DIR=$(echo ${INITRAMFS_STAGING_DIR}/lib/modules/*)
    cp ${MODULES_BASE_DIR}/modules.load ${DIST_DIR}/modules.load
    cp ${MODULES_BASE_DIR}/modules.load ${DIST_DIR}/vendor_boot.modules.load
    echo "${MODULES_OPTIONS}" > ${MODULES_BASE_DIR}/modules.options
