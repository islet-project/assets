...
__attribute__((aligned (PAGE_SIZE))) cloak_print_call gv_cloak_print_call;
...
uint64_t val_realm_cloak_print_call(char *msg, unsigned long data1, unsigned long data2)
{
    size_t len = pal_strlen(msg);

    pal_memset((char *)&gv_cloak_print_call.msg, 0, sizeof(gv_cloak_print_call.msg));
    pal_memcpy((char *)&gv_cloak_print_call.msg, msg, len);
    gv_cloak_print_call.data1 = data1;
    gv_cloak_print_call.data2 = data2;

    return (val_smc_call(RSI_CLOAK_PRINT_CALL, (uint64_t)&gv_cloak_print_call,
                                        0, 0, 0, 0, 0, 0, 0, 0, 0)).x0;
}
