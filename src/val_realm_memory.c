...
/*
    ...
    for (unsigned i=0; i<1; i++) {
        // MAP_REGION_FLAG --> identity mapping!
        unsigned long addr = (unsigned long)val_get_shared_region(ipa_width, shared_ipa_addr_start[i]);
        val_realm_cloak_print_call("shared_addr", addr, 0);

        mmap_region_t plat_shared_region = MAP_REGION_FLAT((uint64_t)addr,
                        shared_ipa_addr_end[i] - shared_ipa_addr_start[i],
                        MT_RW_DATA | MT_NS);
