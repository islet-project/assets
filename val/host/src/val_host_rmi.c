...
{
    return (val_smc_call(RMI_VERSION, 0, 0, 0, 0, 0, 0, 0)).x0;
}
...
uint64_t val_host_rmi_features(uint64_t index, uint64_t *value)
{
    ...
    args = val_smc_call(RMI_FEATURES, index, 0, 0, 0, 0, 0, 0);
...
uint64_t val_host_rmi_rec_aux_count(uint64_t rd, uint64_t *aux_count)
{
    ...
    args = val_smc_call(RMI_REC_AUX_COUNT, rd, 0, 0, 0, 0, 0, 0);
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_data_create(uint64_t data, uint64_t rd,
               uint64_t ipa, uint64_t src, uint64_t flags)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_DATA_CREATE, data, rd, ipa, src, flags, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_data_create_unknown(uint64_t data,
                   uint64_t rd, uint64_t ipa)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_DATA_CREATE_UNKNOWN, data, rd, ipa, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
...
uint64_t val_host_rmi_granule_delegate(uint64_t addr)
{
    ...
    ret = (val_smc_call(RMI_GRANULE_DELEGATE, addr, 0, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
...
uint64_t val_host_rmi_granule_undelegate(uint64_t addr)
{
    ...
    ret = (val_smc_call(RMI_GRANULE_UNDELEGATE, addr, 0, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
    val_host_update_destroy_granule_state(0, addr, GRANULE_UNDELEGATED, 0);
    return ret;
}
...
uint64_t val_host_rmi_psci_complete(uint64_t calling_rec,
                 uint64_t target_rec)
{
    return (val_smc_call(RMI_PSCI_COMPLETE, calling_rec, target_rec, 0, 0, 0, 0, 0)).x0;
}
...
{
    return (val_smc_call(RMI_REALM_ACTIVATE, rd, 0, 0, 0, 0, 0, 0)).x0;
}

uint64_t val_host_rmi_local_channel_setup(uint64_t c_rd, uint64_t s_rd, uint64_t lc_pa, uint64_t lc_ipa, uint64_t lc_size)
{
...
uint64_t val_host_rmi_realm_create(uint64_t rd, uint64_t params_ptr)
{
    ...
    ret = (val_smc_call(RMI_REALM_CREATE, rd, params_ptr, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
...
uint64_t val_host_rmi_realm_destroy(uint64_t rd)
{
    ...
    ret = (val_smc_call(RMI_REALM_DESTROY, rd, 0, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
    val_host_update_destroy_granule_state(rd, rd, GRANULE_DELEGATED, GRANULE_RD);
    return ret;
}
...
uint64_t val_host_rmi_data_destroy(uint64_t rd, uint64_t ipa)
{
    ...
    ret = (val_smc_call(RMI_DATA_DESTROY, rd, ipa, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
    val_host_update_destroy_granule_state(rd, ipa, GRANULE_DELEGATED, GRANULE_DATA);
    return ret;
}
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_rec_create(uint64_t rec, uint64_t rd,
                 uint64_t params_ptr)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_REC_CREATE, rec, rd, params_ptr, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
...
uint64_t val_host_rmi_rec_destroy(uint64_t rec)
{
    ...
    ret = (val_smc_call(RMI_REC_DESTROY, rec, 0, 0, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
    val_host_update_destroy_granule_state(0, rec, GRANULE_DELEGATED, GRANULE_REC);
    return ret;
}
...
rec_enter:
    ret = (val_smc_call(RMI_REC_ENTER, rec, run_ptr, 0, 0, 0, 0, 0)).x0;
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_rtt_create(uint64_t rtt, uint64_t rd,
              uint64_t ipa, uint64_t level)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_RTT_CREATE, rtt, rd, ipa, level, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_rtt_fold(uint64_t rtt, uint64_t rd,
              uint64_t ipa, uint64_t level)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_RTT_FOLD, rtt, rd, ipa, level, 0, 0, 0)).x0;
    {
        return ret;
    }
    val_host_update_destroy_granule_state(rd, rtt, GRANULE_DELEGATED, GRANULE_RTT);
    return ret;
}
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_rtt_destroy(uint64_t rtt, uint64_t rd,
               uint64_t ipa, uint64_t level)
{
    uint64_t ret;

    ret = (val_smc_call(RMI_RTT_DESTROY, rtt, rd, ipa, level, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
                  ...
                  uint64_t level, uint64_t desc)
{
    ...
    ret = (val_smc_call(RMI_RTT_MAP_UNPROTECTED, rd, ipa, level, desc, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
                 ...
                 uint64_t level, val_host_rtt_entry_ts *rtt)
{
    ...
    args = val_smc_call(RMI_RTT_READ_ENTRY, rd, ipa, level, 0, 0, 0, 0);
                    ...
                    uint64_t level)
{
    ...
    ret = (val_smc_call(RMI_RTT_UNMAP_UNPROTECTED, rd, ipa, level, 0, 0, 0, 0)).x0;
    if (ret)
    {
        return ret;
    }
    val_host_update_destroy_granule_state(rd, ipa, GRANULE_UNPROTECTED, GRANULE_UNPROTECTED);
    return ret;
}
 ...
 *   @return   Returns command return status
**/
uint64_t val_host_rmi_rtt_init_ripas(uint64_t rd, uint64_t ipa,
                    uint64_t level)
{
    return (val_smc_call(RMI_RTT_INIT_RIPAS, rd, ipa, level, 0, 0, 0, 0)).x0;
}
...
uint64_t val_host_rmi_rtt_set_ripas(uint64_t rd, uint64_t rec,
                    uint64_t ipa, uint64_t level, uint64_t ripas)
{
    return (val_smc_call(RMI_RTT_SET_RIPAS, rd, rec, ipa, level, ripas, 0, 0)).x0;
}
